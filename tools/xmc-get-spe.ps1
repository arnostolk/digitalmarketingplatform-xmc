...
if (-not ($environmentInfo.cm -and $environmentInfo.spe_username -and $environmentInfo.spe_sharedsecret)) {
    Write-Error "Environment '$environment' missing field 'cm', or 'spe_username', or 'spe_sharedsecret' in file .\tools\xmc-config.json. See .\tools\xmcloud-config.md for an example of the required format."
}
