...
# in 'tools\xmcloud-config.json'. This login makes sure that the XM Cloud login 
...
# authority URL and audience URL) and the configuration file `tools\xmcloud-config.json`
...
if (-not ($environmentInfo.id -and $environmentInfo.cm)) {
    Write-Error "Environment '$environment' missing field 'id' or 'cm' in file .\tools\xmcloud-config.json. See .\tools\xmcloud-config.md for an example of the required format."
}
...
try {
    ...
    Write-Error "The XM Cloud CM URL $cmUrl does not responded with a 200 status code. Check your configuration for environment '$environment' in '.\tools\xmcloud-config'."
}
